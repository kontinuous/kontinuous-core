...
object UIController {
    ...
    val loginPost = Action ({ ctx ->
        ...
        val cookies = ctx.requestHeaders.cookies.keySet().fold("", { s, m -> s + ";" + m })
        println("Header cookies: " + cookies)
...
class UIAuthenticationTest {
    ...
    Test fun shouldHaveCookie() {
        ...
        var cookies = driver.manage()!!.getCookies()!!.fold("", { s, c -> s + ";" + c.getName() })
        println("Cookies: " + cookies)
        assertNotNull(driver.manage()?.getCookieNamed("id"))
        driver.get("http://localhost:8080/redirect2")
        cookies = driver.manage()!!.getCookies()!!.fold("", { s, c -> s + ";" + c.getName() })
        println("Cookies: " + cookies)
    ...
    Test fun shouldBeLoggedIn() {
        ...
        elemSubmit?.submit()
        val cookies = driver.manage()!!.getCookies()!!.fold("", {s, c -> s + ";" + c.getName()})
        println("Cookies: " + cookies)
