...
object MimeTypes {
    ...
    fun forExtension(ext: String): String? {
        return defaultTypes.get(ext)
    }

    fun forFileName(name: String): String? {
        val splitted = name.split('.')
        return if(splitted.size > 0) {
            forExtension(splitted[splitted.lastIndex])
        } else {
            null
        }
    }
    ...
    """.split('\n').map { line -> line.trim() }
                   ...
                   .map { line -> line.split('=') }
                   .fold(HashMap<String, String>(), { (result, value) -> result.put(value[0], value[1]); result })
}